...
void FragmentedMp4Writer::FlushFragment() {
    ...
    size_t videoDataOffsetPatch = 0;
    size_t audioDataOffsetPatch = 0;
    ...
    if (!video_.empty()) patch32(videoDataOffsetPatch, videoDataStart);
    if (!audio_.empty()) patch32(audioDataOffsetPatch, audioDataStart);
